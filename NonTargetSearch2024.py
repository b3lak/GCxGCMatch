...
class App(QWidget):
    ...
    def process_file(self):
        ...
        result_dfs = {
            "Area": df_first[["Compound_" + first_sheet, "RT1_" + first_sheet, "RT2_" + first_sheet, "Major_" + first_sheet, "Qual_" + first_sheet]].copy(),
        ...
        for sheet_name in xls.sheet_names[1:]:
            ...
            for index, row in df_first.iterrows():
                matched_row_area = df[(df[f"RT1_{sheet_name}"].between(row[f"RT1_{first_sheet}"] - tolerance, row[f"RT1_{first_sheet}"] + tolerance)) &
                                    (df[f"RT2_{sheet_name}"].between(row[f"RT2_{first_sheet}"] - tolerance, row[f"RT2_{first_sheet}"] + tolerance)) &
                                    (df[f"Major_{sheet_name}"].between(row[f"Major_{first_sheet}"] - tolerance, row[f"Major_{first_sheet}"] + tolerance)) &
                                    (df[f"Qual_{sheet_name}"].between(row[f"Qual_{first_sheet}"] - tolerance, row[f"Qual_{first_sheet}"] + tolerance))]["Area_" + sheet_name].values
                                    
                #matched_row_height = df[(df[f"RT1_{sheet_name}"].between(row[f"RT1_{first_sheet}"] - tolerance, row[f"RT1_{first_sheet}"] + tolerance)) &
                                    #(df[f"RT2_{sheet_name}"].between(row[f"RT2_{first_sheet}"] - tolerance, row[f"RT2_{first_sheet}"] + tolerance)) &
                                    #(df[f"Major_{sheet_name}"].between(row[f"Major_{first_sheet}"] - tolerance, row[f"Major_{first_sheet}"] + tolerance))]["Height_" + sheet_name].values

                #matched_row_stn = df[(df[f"RT1_{sheet_name}"].between(row[f"RT1_{first_sheet}"] - tolerance, row[f"RT1_{first_sheet}"] + tolerance)) &
                                    #(df[f"RT2_{sheet_name}"].between(row[f"RT2_{first_sheet}"] - tolerance, row[f"RT2_{first_sheet}"] + tolerance)) &
                                    #(df[f"Major_{sheet_name}"].between(row[f"Major_{first_sheet}"] - tolerance, row[f"Major_{first_sheet}"] + tolerance))]["Signal to Noise_" + sheet_name].values

                #matched_row_area_percent = df[(df[f"RT1_{sheet_name}"].between(row[f"RT1_{first_sheet}"] - tolerance, row[f"RT1_{first_sheet}"] + tolerance)) &
                                            #(df[f"RT2_{sheet_name}"].between(row[f"RT2_{first_sheet}"] - tolerance, row[f"RT2_{first_sheet}"] + tolerance)) &
                                            #(df[f"Major_{sheet_name}"].between(row[f"Major_{first_sheet}"] - tolerance, row[f"Major_{first_sheet}"] + tolerance))]["Area %_" + sheet_name].values
        ...
        with pd.ExcelWriter(output_file_name) as writer:  # <-- Here's the fix
